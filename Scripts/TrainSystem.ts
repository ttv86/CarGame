...
export default class TrainSystem {
    ...
    constructor(game: Game, gameMap: IGameMap, style: IStyle) {
        this.gameMap = gameMap;
        const model = style.getVehicleModelByType("train");
        if (model === null) {
        ...
        const trainInfo = style.getVehicleInfo(model);
        if (!trainInfo) {
            console.error("Can't find train model");
            this.routeBlocks = [];
            return;
...
function findRoute(gameMap: IGameMap): TileReference[] {
